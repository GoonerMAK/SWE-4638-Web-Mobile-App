...
const page = () => {
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
        try {
            const response = await axios.get('http://localhost:5000/fetchHistoryQuizData'); 
  ...
  const onOptionSelected = (option, idx) => {
    ...
    if (option === historyQuizData[activeQuestion]?.correctAnswer) {
  ...
  const nextQuestion = () => {
    ...
    if (activeQuestion !== historyQuizData.length - 1) {
      ...
      <div>
        {questionTracker ? (
          <h2>
            ...
            <span>/{historyQuizData.length}</span>
          </h2>
        ) : ( 
          <span> </span>
        )}
      ...
      <div>
        {!showResult ? (
          <div className='quiz-container'>
            ...
            <h3>{historyQuizData[activeQuestion]?.question}</h3>
            {historyQuizData[activeQuestion]?.options.map((option, idx) => (
              <div>
              <button
                key={idx}
            ...
            {checked ? (
              <button onClick={nextQuestion} className='btn'>
                {activeQuestion === historyQuizData.length - 1 ? 'Finish' : 'Next'}
              </button>
            ) : (
          ...
          <div className='quiz-container'>
            <h3>Results</h3>
            <h3>Overall { ((result.score / historyQuizData.length) * 100).toFixed(2) }%</h3>
            <p>
              Total Questions: <span>{historyQuizData.length}</span>
            </p>
            <p>
            ...
            <div {...getCollapseProps()}>
              {historyQuizData.map((question, idx) => (
                <div key={idx}>
